...
int main (int argc, char* argv[])
{
	...
	if (argc >= 2)
	{
		if (argc > 2) cout << "proj10: Ignoring extra tokens.\n";
		...
		while (tick < ticks)
		{
			...
			if (isRunning)
			{
				...
				// CHECK IF DONE WITH TIME QUANTUM
				else if (Running.quant == quantum)
				{
					Running.quant = 0;
					Running.enteredReady = tick;
					Ready.push_back(Running);
					isRunning = false;
					cout << "PID " << Running.ID << ": Running to Ready\n";
				}
			...
			// based on short term scheduling algorithm
			int minID = 100000;
			int minIndex = 0;
			if ((!isRunning) && !(Ready.empty()))
			{
				...
				if ((alg == "FCFS") || (alg == "RR"))
				{
					for (unsigned int i = 0; i < Ready.size(); i++)
					{
						Ready[i].cReady++;
						if ((minID == 100000) || 
								(Ready[i].enteredReady < Ready[minID].enteredReady))
						{
							if (Ready[i].ID < minID)
							{
								minID = Ready[i].ID;
				...
				// priority short term scheduling
				else if (alg == "Priority")
				{
					for (unsigned int i = 0; i < Ready.size(); i++)
					{
						Ready[i].cReady++;
						if ((minID == 100000) || 
								(Ready[i].priority < Ready[minID].priority))
						{
							if (Ready[i].ID < minID)
							{
								minID = Ready[i].ID;
	...
	{
		cout << "proj10: Error: No input file given.\n";
		exit(1);
	}
